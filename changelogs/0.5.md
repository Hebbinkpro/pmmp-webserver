...
- Fix major bug in `ThreadSafeUtils::unwrapThreadSafe()` resulting in NULL values throwing an exception while
  unwrapping.
    - `null` is now a valid type to unwrap.
...
- Fix bug where SocketClient became unavailable, crashing the WebServer

## v0.5.4

### Security Release

- **Affected versions:** All versions up to and including v0.5.3
- Fixed a critical security vulnerability in `StaticRoute` that could allow unauthorized access to files outside the
  configured static directory. The issue was resolved by enforcing strict canonical path validation for static file
  requests.
